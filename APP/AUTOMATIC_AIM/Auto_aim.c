...
//	PID_ControllerInit(&auto_pit,50,50,10,0.001);
    ...
    PID_ControllerInit(&auto_pit,2,2,60,0.001);

    PID_SetGains(&auto_yaw,0.0224,0,0);//0.000018);//0.0025,0.001,0 0.0025,0,0.000008

#ifdef INFANTRY_2
    PID_SetGains(&auto_pit,0.05,0,0.001);//0.0086,0,0
#endif
#ifdef INFANTRY_1
    PID_SetGains(&auto_pit,0.015,0,0.001);//0.0086,0,0
#endif
#ifdef INFANTRY_3
    PID_SetGains(&auto_pit,0.015,0,0.001);//0.0086,0,0
#endif
...
void Auto_aim(frame fram,int len)
{
    ...
    if(timeout_count > 1000&&big_power_flag == 0xFF)
    {
        //fram.extra[0] = 0x33;
...
//
    ...
    else if(big_power_flag == 0xFF && auto_aim_flag == 0 && fram.extra[0]==0x34 && timeout_count < 1000)//原值3000
    {
        if(auto_aim_renew_flag==1)
        {
#ifdef INFANTRY_2
