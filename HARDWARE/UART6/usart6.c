 ...
 *Version	: V1.0
 *Update	: 2017.12.11
 *Description: 	Usart6 for common use
				PG14 -> USART3_TX
				PG9 -> USART3_RX
 ************************************************************/
...
u8 USART6_dma[USART6_dma_rx_len];		//DMA接收数据
u8 Personal_Data[USART6_dma_tx_len];	//DMA发送数据
...
/* -------------- Configure DMA -----------------------------------------*/
	...
	dma.DMA_BufferSize = USART6_dma_rx_len;       //???
	...
	dma.DMA_BufferSize = USART6_dma_tx_len;       			//要改？
	dma.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	dma.DMA_MemoryInc = DMA_MemoryInc_Enable;
	dma.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
	dma.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
