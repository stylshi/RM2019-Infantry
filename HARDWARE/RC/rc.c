...
void USART1_IRQHandler(void)
{
	if (USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
	{
		...
		this_time_rx_len = USART1->DR;
		this_time_rx_len = DMA_GetCurrDataCounter(DMA2_Stream2); 
		DMA_Cmd(DMA2_Stream2, DISABLE);
//		this_time_rx_len = /*SBUS_RX_BUF_NUM*/18 - DMA_GetCurrDataCounter(DMA2_Stream2);
...
//		else
		...
		DMA_SetCurrDataCounter(DMA2_Stream2,18);  //设置传输数据长度
...
void ItoE(void)
{
	  ...
	  RC_Ex_Ctl.key.v = RC_Ctl.key.v;
		...
		{
		if(	friction_rc_last_position == 1 && RC_Ex_Ctl.rc.s1 == 3 && RC_Ex_Ctl.rc.s2 == 1)   //判断遥控器s1是否1、3位置切换过
